...
use web_sys::{console, HtmlInputElement as InputElement};
...
impl Component for App {
    ...
    fn create(_ctx: &Context<Self>) -> Self {
        let entries: Vec<Entry> = LocalStorage::get(KEY).unwrap_or_else(|_| Vec::new());
        let state = State {
            entries,
            filter: Filter::All, 
            edit_value: "".into()
            ...
            <div class="todomvc-wrapper">
                <section class="todoapp">
                    ...
                    <section class={classes!("main", hidden_class)}>
                        ...
                        <ul class="todo-list">
                        {
                            for self.state.entries.iter().filter(|e| self.state.filter.fits(e)).enumerate().map(|f| self.view_entry(f, ctx.link()))
                        }
                        </ul>
                    </section>
...
impl App {
    fn view_input(&self, link: &Scope<Self>) -> Html {
        let onkeypress = link.batch_callback(|e: KeyboardEvent| {
            if e.key() == "Enter" {
                ...
                // this is how u console.log
                console::log_2(&value.clone().into(), &"here".into());
    ...
    fn view_entry(&self, (idx, entry): (usize, &Entry), link: &Scope<Self>) -> Html {
        ...
        html! {
            <li {class}>
                <div class="view">
                    <input type="checkbox" class="toggle" checked={entry.completed} onclick={link.callback(move |_| Msg::Toggle(idx) )}/>
                    <label ondblclick={(link.callback(move |e| Msg::ToggleEdit(idx)))}>{ &entry.description }</label>
    ...
    fn view_filter(&self, filter: Filter, link: &Scope<Self>) -> Html {
        let class = if self.state.filter == filter { "selected "} else { "not-selected" };
        html! {
            <li>
                <a 
                    class={class}
